...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = useState(() => { try { return JSON.parse(localStorage.getItem("scr_bm") || "[]"); } catch { return []; } });
  const [chat, setChat] = useState([]);
  const [chatQ, setChatQ] = useState("");
  const [chatBusy, setChatBusy] = useState(false);
  const [open, setOpen] = useState({ ai: true, files: false });

  useEffect(() => { const r = () => setMobile(window.innerWidth < 700); window.addEventListener("resize", r); return () => window.removeEventListener("resize", r); }, []);
  ...
  const analyze = async () => {
    ...
    try {
      const res = await axios.post("https://smart-code-review-platform.onrender.com/api/github/analyze", { repoUrl });
  ...
  const sendChat = async () => {
    ...
    const q = chatQ; setChat(p => [...p, { r: "u", t: q }]); setChatQ(""); setChatBusy(true);
    try {
      const res = await axios.post("https://smart-code-review-platform.onrender.com/api/github/chat", {
        question: q,
        repoContext: { repo: result.repo, language: result.language, stars: result.stars, projectType: result.projectType, issues: result.issues.map(i => i.message), strengths: result.strengths, documentationScore: result.documentationScore, structureScore: result.structureScore, codeScore: result.codeScore, overallScore: result.overallScore, aiReview: result.aiReview },
      });
      setChat(p => [...p, { r: "a", t: res.data.answer }]);
    } catch { setChat(p => [...p, { r: "a", t: "Couldn't get a response. Please try again." }]); }
    finally { setChatBusy(false); }
  };

  const toggleBM = (d) => setBookmarks(p => { const e = p.find(b => b.repo === d.repo); const u = e ? p.filter(b => b.repo !== d.repo) : [d, ...p]; localStorage.setItem("scr_bm", JSON.stringify(u)); return u; });
  ...
  const fmt = (text) => text.split("\n").map((line, i) => {
    ...
    if (s.endsWith(":") && s.length < 42 && !s.startsWith("-")) return <div key={i} style={{ fontWeight: 600, fontSize: 12, color: t.teal, marginTop: 12, marginBottom: 4, paddingBottom: 4, borderBottom: `1px solid ${t.border}` }}>{s}</div>;
    if (s.startsWith("-") || s.startsWith("•")) return <div key={i} style={{ display: "flex", gap: 7, marginBottom: 4, paddingLeft: 2 }}><span style={{ color: t.teal, flexShrink: 0, marginTop: 1 }}>›</span><span>{s.replace(/^[-•]\s*/, "")}</span></div>;
    if (/^\d+\./.test(s)) return <div key={i} style={{ display: "flex", gap: 7, marginBottom: 4, paddingLeft: 2 }}><span style={{ color: t.teal, fontWeight: 600, minWidth: 15 }}>{s.match(/^\d+/)[0]}.</span><span>{s.replace(/^\d+\.\s*/, "")}</span></div>;
          ...
          <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 24px", height: 54, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              <div style={{ width: 28, height: 28, background: "linear-gradient(135deg,#0d9488,#0f766e)", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M2 8h4M8 2v4M8 10v4M10 8h4" stroke="#fff" strokeWidth="1.8" strokeLinecap="round"/><circle cx="8" cy="8" r="2" fill="#fff"/></svg>
              </div>
              <span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 700, fontSize: 14, letterSpacing: "-0.2px", color: t.text }}>CodeReview.ai</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 12, color: t.sub }}>Hi, {user.name}</span>
              <button onClick={() => setDark(!dark)} style={{ padding: "5px 12px", borderRadius: 7, border: `1px solid ${t.border}`, background: "transparent", color: t.sub, cursor: "pointer", fontSize: 12 }}>{dark ? "Light" : "Dark"}</button>
              <button onClick={() => setUser(null)} style={{ padding: "5px 12px", borderRadius: 7, border: `1px solid ${t.border}`, background: "transparent", color: t.sub, cursor: "pointer", fontSize: 12 }}>Sign out</button>
            </div>
          </div>
        </header>
            ...
            <div id="report" className="fu">
                ...
                <div style={{ display: "flex", flexDirection: mobile ? "column-reverse" : "row", gap: 20, justifyContent: "space-between" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginBottom: 5 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 999, background: result.healthColor === "green" ? "rgba(16,185,129,0.1)" : result.healthColor === "yellow" ? "rgba(245,158,11,0.1)" : "rgba(244,63,94,0.1)", color: result.healthColor === "green" ? t.green : result.healthColor === "yellow" ? t.amber : t.red, border: `1px solid ${result.healthColor === "green" ? "rgba(16,185,129,0.2)" : result.healthColor === "yellow" ? "rgba(245,158,11,0.2)" : "rgba(244,63,94,0.2)"}` }}>
                        {result.healthStatus}
                      </span>
                    </div>
                    <p style={{ fontSize: 12, color: t.sub, marginBottom: 18 }}>{result.language} · ⭐ {result.stars} · {result.totalFiles} files · {result.projectType}</p>
              ...
              <Card theme={t}>
                ...
                {open.files && (
                  <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 8, fontSize: 13, color: t.sub }}>
                    {result.pythonFiles > 0 && <div>🐍 Python: {result.pythonFiles}{result.pythonPreview?.map((f, i) => <div key={i} style={{ fontSize: 11, marginLeft: 14, marginTop: 2, color: t.sub }}>· {f}</div>)}</div>}
                    {result.jsFiles > 0 && <div>📦 JS: {result.jsFiles}{result.jsPreview?.map((f, i) => <div key={i} style={{ fontSize: 11, marginLeft: 14, marginTop: 2, color: t.sub }}>· {f}</div>)}</div>}
                    {result.notebookFiles > 0 && <div>📓 Notebooks: {result.notebookFiles}</div>}
                    {result.csvFiles > 0 && <div>📊 CSV: {result.csvFiles}</div>}
                    <div style={{ color: t.sub }}>📁 Total: {result.totalFiles} files</div>
                  </div>
                )}
              </Card>
              ...
              <Card theme={t}>
                    ...
                    <div style={{ fontSize: 13, color: t.sub }}>
                      ...
                      <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {["How to improve this?", "Biggest risks?", "Production ready?"].map(s => (
                          <span key={s} onClick={() => setChatQ(s)} style={{ padding: "4px 12px", borderRadius: 999, border: `1px solid ${t.border}`, fontSize: 12, cursor: "pointer", color: t.sub, transition: "all 0.15s" }}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input value={chatQ} onChange={e => setChatQ(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !chatBusy) sendChat(); }}
                    ...
                    {result.issues.filter(i => filter === "All" || i.severity === filter).length === 0
                      ...
                      : result.issues.filter(i => filter === "All" || i.severity === filter).map((i, idx) => (
                        <li key={idx} style={{ color: i.severity === "high" ? t.red : i.severity === "medium" ? t.amber : t.green, marginBottom: 7, display: "flex", gap: 6, lineHeight: 1.5 }}>
                          <span style={{ flexShrink: 0 }}>{i.severity === "high" ? "●" : i.severity === "medium" ? "●" : "●"}</span>{i.message}
                        </li>
                      ))}
                  </ul>
                </Card>
